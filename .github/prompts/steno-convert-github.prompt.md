...
Default mode: `preview` unless user explicitly reqs apply.
...
- preserve heading and list order unless user reqs restructuring
- use stable shorthand only: `cfg`, `auth`, `deps`, `env`, `req`, `resp`, `impl`, `perf`, `arch`, `ctx`, `ctr`
- if a file already appears steno-compressed, skip w/ note
